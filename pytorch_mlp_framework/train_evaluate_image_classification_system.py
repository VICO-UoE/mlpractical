...
if __name__ == "__main__":
    ...
    train_worker = 1 if sys.platform == 'win32' else 2      # For training, it is worth it to spawn on Windows despite the cost
    other_worker = 0 if sys.platform == 'win32' else 2      # For validation/testing, spawning cost outweights benefit
    train_data_loader = DataLoader(train_data, batch_size=args.batch_size, shuffle=True, num_workers=train_worker, multiprocessing_context=context)
    val_data_loader = DataLoader(val_data, batch_size=args.batch_size, shuffle=True, num_workers=other_worker, multiprocessing_context=context)
    test_data_loader = DataLoader(test_data, batch_size=args.batch_size, shuffle=True, num_workers=other_worker, multiprocessing_context=context)
